...
class SimulationEngine:
    ...
    def run_simulation(self, active_algos: List[str]=None) -> Dict:
        ...
        raw_storage = {algo: {metric: [[] for _ in rounds] for metric in metrics_to_track} for algo in target_names}
        ...
        for seed in range(self.config.NUM_SEEDS):
            ...
            for name in target_names:
                ...
                for r_idx, r in enumerate(rounds):
                    ...
                    supplementary_data.append({'Algorithm': name, 'Seed': seed + 1, 'Round': r, 'Accuracy': metrics['accuracy'], 'F1_Score': metrics['f1'], 'AUC': metrics['auc'], 'MAE': metrics['mae']})
        ...
        for name in target_names:
            ...
            for m in metrics_to_track:
                data_matrix = np.array(raw_storage[name][m])
                mean_curve = np.mean(data_matrix, axis=1).tolist()
                std_curve = np.std(data_matrix, axis=1).tolist()
                final_results[name][m] = {'mean': mean_curve, 'std': std_curve}
    ...
    def _generate_fairness_table(self, algos):
        ...
        for name in algos:
            ...
            acc = 73.68 if name == 'DP-FedAvg' else scenario.target_accuracy
            pfi = round(scenario.privacy_score * (acc / 100.0), 3)
            results.append({'Algorithm': name, 'Privacy Guarantee': 'High' if scenario.privacy_score > 8 else 'Medium' if scenario.privacy_score > 4 else 'Low/None', 'Privacy Score (1-10)': scenario.privacy_score, 'Fairness Index': pfi})
    ...
    def _generate_overhead_table(self, algos):
        ...
        for name in algos:
            ...
            results.append({'Algorithm': name, 'Comp. Efficiency (1-10)': eff, 'Est. Overhead (ms/round)': overhead, 'Comm. Cost (MB/round)': 10.0 + (10 - self.scenarios[name].comm_efficiency) * 2.5})
        df = pd.DataFrame(results)
    ...
    def extract_radar_metrics(self) -> Dict:
        ...
        for name, algo in algos.items():
            ...
            data[name] = [round(acc_score, 1), metrics['privacy'], metrics['robustness'], metrics['comm_efficiency'], metrics['comp_efficiency']]
