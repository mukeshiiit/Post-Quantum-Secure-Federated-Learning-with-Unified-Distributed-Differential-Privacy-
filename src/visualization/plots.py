...
class Visualizer:
    ...
    def __init__(self, output_dir: str):
        ...
        self.styles = {'PQ-FL (Proposed)': {'color': '#d62728', 'linewidth': 2.5, 'linestyle': '-', 'marker': ''}, 'Vanilla FedAvg': {'color': '#1f77b4', 'linewidth': 1.5, 'linestyle': '--', 'marker': ''}, 'FedProx': {'color': '#2ca02c', 'linewidth': 1.5, 'linestyle': '-.', 'marker': ''}, 'SCAFFOLD': {'color': '#9467bd', 'linewidth': 1.5, 'linestyle': ':', 'marker': ''}, 'DP-FedAvg': {'color': '#ff7f0e', 'linewidth': 1.5, 'linestyle': '-', 'marker': ''}, 'FL + Distributed Noise': {'color': '#8c564b', 'linewidth': 1.5, 'linestyle': '--', 'marker': ''}, 'FL + Secure Aggregation': {'color': '#e377c2', 'linewidth': 1.5, 'linestyle': '-.', 'marker': ''}, 'FL + PQ Encryption Only': {'color': '#7f7f7f', 'linewidth': 1.5, 'linestyle': ':', 'marker': ''}}

    def _plot_metric_with_error_bars(self, metrics_data: Dict, metric_key: str, title: str, ylabel: str, filename: str):
    ...
    def plot_all_metrics(self, metrics: Dict):
        self._plot_metric_with_error_bars(metrics, 'accuracy', 'Convergence Analysis: Accuracy vs Rounds', 'Test Accuracy (%)', 'comparison_accuracy.png')
        self._plot_metric_with_error_bars(metrics, 'f1', 'F1-Score Analysis (Mean ± Std)', 'F1 Score', 'comparison_f1.png')
        self._plot_metric_with_error_bars(metrics, 'auc', 'Area Under Curve (AUC) vs Rounds', 'AUC Score', 'comparison_auc.png')
        self._plot_metric_with_error_bars(metrics, 'mae', 'Mean Absolute Error (MAE) vs Rounds', 'Mean Absolute Error', 'comparison_mae.png')
    ...
    def plot_tradeoff_scatter(self, metrics: Dict, radar_data: Dict):
        ...
        plt.text(9.5, 89, 'OPTIMAL ZONE\nHigh Privacy + High Accuracy', fontsize=11, fontweight='bold', color='darkgreen', ha='center', bbox=dict(facecolor='#e6f5e6', edgecolor='green', boxstyle='round,pad=0.5'))
        offsets = {'PQ-FL (Proposed)': (-60, 10), 'Vanilla FedAvg': (0, 15), 'FedProx': (0, -25), 'SCAFFOLD': (0, 15), 'DP-FedAvg': (0, 15)}
        for i, txt in enumerate(labels):
            off = offsets.get(txt, (0, 10))
            plt.annotate(txt, (x_priv[i], y_acc[i]), xytext=off, textcoords='offset points', ha='center', fontsize=11, fontweight='bold', bbox=dict(boxstyle='round,pad=0.3', fc='white', ec='black', alpha=0.8))
        ...
        plt.legend([l1, l2, l3], ['Low Efficiency', 'Med Efficiency', 'High Efficiency'], title='Comm. Efficiency (Bubble Size)', scatterpoints=1, frameon=True, labelspacing=1, loc='lower left', fontsize=9)
        plt.tight_layout()
